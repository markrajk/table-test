...
const mapDispatchToProps = (dispatch: AppDispatch) => ({
  ...
  getUserTextFeedback: (userId: string) =>
    dispatch(getUserTextFeedback(userId, 'subordinate')),
...
const GeneralSettings = (props: Props) => {
  ...
  useEffect(() => {
    if (selectedTeam) {
      ...
      setTimeout(() => getActiveUsers(selectedTeam._id, 'teamLeaders'), 200)
    }
  }, [selectedTeam])
  ...
  useEffect(() => {
    if (
      ...
      const filtered = selectedTeamData?.teamMembers.filter(
        (tm) =>
          !selectedTeamData.teamLeaders.some((lead) => lead._id === tm._id)
      )
      if (filtered.length) {
        selectUserState(filtered[0])
      } else {
            ...
            <div className="questions-main">
              {selectedTeamData?.teamMembers &&
                ...
                selectedTeamData?.teamMembers
                  .filter(
                    (tm) =>
                      !selectedTeamData.teamLeaders.some(
                        (lead) => lead._id === tm._id
                      )
                  )
                  .map((item) => (
                    <div
          ...
          <div className="content-card answers">
            <div className="answers-header">
              <p className="answers-header-title">
                {t('textFeedbackPages:Feedbackgivento')}  {' '}
                {selectedUser?.firstname + ' ' + selectedUser?.lastname}
              </p>

              <p className="answers-header-amount">
                {t("textFeedbackPages:Fromteamleaders")}
              </p>
            </div>
                ...
                userTextFeedback.map((feed) => (
                  ...
                  <div className="answers-placeholder">
                    ...
                    <p className="answers-placeholder-text" style={{ maxWidth: '23rem' }}>
                      {t("textFeedbackPages:Therehasnotyetbeengiventeamleader")}{' '}
